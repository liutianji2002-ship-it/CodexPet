...
- 顶部左侧状态芯片从旧的 `Direct` 改成了真实的 `AX`
- 这个芯片现在直接反映 `CodexPet.app` 自己是否还具备辅助功能权限
- 给侧边栏 unread / running 结果增加了降值保护，避免一次瞬时少抓就把数字直接压低
- Codex 回到前台时会更积极地刷新状态
- 明确了失败模式：如果 `AX` 是 `Off`，那 unread 数字不准是预期结果，因为宠物已经无法读取 Codex 左侧 UI
