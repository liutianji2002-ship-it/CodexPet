...
CodexPet 当前依赖两类 macOS 权限：

- `辅助功能`：用于读取侧边栏、判断运行中线程、识别当前活跃线程
- `屏幕录制`：用于在 Codex 的 AX 树不完整时，直接按窗口像素识别蓝点

授予步骤：

1. 打开 `系统设置`
2. 进入 `隐私与安全性`
3. 打开 `辅助功能`，把 `CodexPet.app` 打开
4. 打开 `屏幕录制`，把 `CodexPet.app` 打开
5. 完全退出并重新打开 `CodexPet.app`

如果列表里还没有 `CodexPet.app`：

1. 先启动一次 `/Users/<你的用户名>/Applications/CodexPet.app`
2. 如果有旧的失效条目，先删掉，再把当前 app 重新加进去
3. 回到权限页重新勾选

注意：

- 替换或重新安装 `CodexPet.app` 之后，macOS 可能会把它当成一个新的授权对象
- 如果顶部状态变成 `AX Off`，优先回到 `辅助功能` 页面检查
- 如果升级后蓝点 unread 识别明显变差，优先回到 `屏幕录制` 页面检查
